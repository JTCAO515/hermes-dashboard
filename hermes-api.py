...
class HermesAPIHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        elif path.startswith("/api/hermes/conversations/"):
            # /api/hermes/conversations/<session_id>
            sid = path.replace("/api/hermes/conversations/", "")
            data = self.get_conversation_detail(sid)
    ...
    def get_conversations_summary(self):
        """Lightweight session summary for /all endpoint (no message bodies)"""
        conv_file = os.path.join(HERMES_DIR, "data", "conversations", "conversations.json")
        if not os.path.exists(conv_file):
            return {"total_sessions": 0, "total_messages": 0, "latest": []}
        try:
    ...
    def get_conversations(self):
        """Return full session list (without message bodies)"""
        conv_file = os.path.join(HERMES_DIR, "data", "conversations", "conversations.json")
        if not os.path.exists(conv_file):
        ...
        try:
            with open(conv_file, encoding="utf-8") as f:
                data = json.load(f)
            # Strip message bodies from all sessions for bandwidth
            for s in data.get("sessions") or []:
                if "messages" in s:
                    s["msg_preview"] = [{"role": m["role"], "time": m["time"], "preview": m.get("content","")[:120]} for m in s["messages"]]
                    del s["messages"]
            return data
        except Exception as e:
            return {"error": str(e)}

    def get_conversation_detail(self, session_id):
        """Return full messages for a single session"""
        conv_file = os.path.join(HERMES_DIR, "data", "conversations", "conversations.json")
        if not os.path.exists(conv_file):
            return {"error": "no conversation data"}
        try:
